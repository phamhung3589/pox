...
var.set('NoRMaL')
color = StringVar()
networkStatus = Label(root,textvariable=var,fg= "red",font=("Helvetica", 70))
networkStatus.pack()
...
################# v.2 #######################
      ...
      new_df = pd.DataFrame([[ent_ip_src,ent_tp_src,ent_tp_dst,ent_packet_type,total_packets]], columns=['ent_ip_src', 'ent_tp_src', 'ent_tp_dst', 'ent_packet_type', 'total_packets'])
      # with open('feature_tcp_bo_cong_an.csv', 'a') as f:
      with open('feature_icmp.csv', 'a') as f:
          new_df.to_csv(f, index=False, header=False)
      # print "Feature list \n ", feature_vector
      # feature_vector.to_pickle('feature_vector')
      # print "Feature list \n ", feature_vector
      # normalize(feature_vector)
      # print "Feature list \n ", feature_vector
      # tobeClassifed = feature_vector.values

      # print "ATK OR NOR ? \t",knn.calculate(tobeClassifed)
      # if knn.calculate(tobeClassifed):
      #     for i in range(0,3):
      #         print "ATTACK!!!"
      ...
      rowForDuc = pd.Series([req2rep,len(df2)], index = ['req2rep','tableSize'])
      # print('row Duc = ',rowForDuc)
      global statsForDuc
      statsForDuc = statsForDuc.append(rowForDuc, ignore_index = True)
      # print('statsForDuc =',statsForDuc)
      if cnt==100:
          statsForDuc.to_pickle("statsForDuc")
...
def launch ():
  ...
  cnt=0
  mean = pd.read_pickle("./somInput/max_feature.pickle")
  std = pd.read_pickle("./somInput/min_pickle.pickle")
  start=t.time()
  print 'start=',start
  ...
  global statsForDuc
  statsForDuc = pd.DataFrame(columns=['req2rep','tableSize'])
